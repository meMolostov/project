...
def test_filter_default_state(sample_transactions: list[dict]) -> None:
...
def test_filter_empty_result(sample_transactions: list[dict]) -> None:
...
def test_filter_with_different_states(sample_transactions: list[dict], state: str, expected: list[dict]) -> None:
    """Параметризованный тест для разных значений state"""
    assert filter_by_state(sample_transactions, state) == expected


def test_sort_by_date_default(date_list: list[dict]) -> None:
...
def test_sort_by_date_increase(date_list: list[dict]) -> None:
