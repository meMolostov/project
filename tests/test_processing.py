...
def test_filter_with_different_states(sample_transactions, state, expected):
    """Параметризованный тест для разных значений state"""
    assert filter_by_state(sample_transactions, state) == expected


def test_sort_by_date_default(date_list):
...
def test_sort_by_date_increase(date_list):
    """Тестирование фильтрации по возрастанию date"""
    assert sort_by_date(date_list,False) == [{'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'},
                                       {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
                                       {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'},
